    ...
    <div class="sidebar-container">
        <nav class="sidebar-nav">
            <div class="top-buttons">
                ...
                >
                    <button
                        @click="openPanel(item.name as any)"
        ...
        <div
            class="w-[260px] absolute border-r border-l border-[#e2e2e2] bg-white h-full left-[44px] z-[999]"
            v-if="showVar"
        >
        ...
        <div
            class="w-[400px] absolute border-r border-[#e2e2e2] bg-white h-full left-[304px] z-[999]"
            v-if="setVarRef"
        >
            <div class="flex justify-end p-3 box-border">
                <el-button type="primary" plain size="small" @click="saveEditor"
                    >保存<span class="text-red-500" v-if="isChange"
                        >*</span
                    ></el-button
                >
            </div>
            ...
            <div class="p-[5px]">
                ...
                <div
                    ...
                    class="w-full h-[200px] border border-[#c9c8c8]"
                ></div>
            </div>
        </div>
        ...
        <div
            v-if="showJS"
            class="w-[600px] absolute border-r border-[#e2e2e2] bg-white h-full left-[44px] z-[999]"
        >
        ...
        <div
            v-if="showRef"
            class="w-[600px] absolute bg-white border-r border-[#e2e2e2] h-full left-[44px] z-[999]"
        >
            <div class="flex justify-between items-center p-[2px_5px]">
...
import { baseComponentList, seniorComponentList } from '@/mool/utils';
...
const openPanel = (
    panel: '+' | 'schema' | 'JS' | 'ref' | 'var' | 'setVar' | 'Page',
) => {
    // 检查当前点击的面板是否已经打开
...
const saveEditor = () => {
        ...
        function findFetchData(list: Page | Col) {
            list.children?.forEach((child, index, li) => {
                if (
                    ...
                    PageSchema.value.ref.searchParams.value[
    ...
    if (showRef.value) {
        let config = refs.editor.getContent();
        const regex =
            /(const|let|var)\s+(\w+)\s*=\s*vue\.(ref|computed|reactive)\s*\(([\s\S]*?)(?:\)\s*;|\)(?=\s*$))/g;
        let matches;
        const results = [];
        ...
        while ((matches = regex.exec(config)) !== null) {
            ...
            try {
                ...
                // 如果解析失败，保留原始字符串
                console.warn(
                    `Failed to parse value for ${variableName}: ${e.message}`,
                );
            }
        ...
        function findFetchData(list: Page | Col) {
            list.children?.forEach((child, index, li) => {
                if (
                    ...
                    child.fetchData.params =
                        PageSchema.value.ref?.searchParams[
                            child.fetchData.order
                        ];
                } else {
                    findFetchData(child as Col);
