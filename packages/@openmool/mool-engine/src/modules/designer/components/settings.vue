<template>
    <div class="propSetting flex">
            ...
            <el-tabs v-model="activeName" style="height: 100%">
                <el-tab-pane label="属性" name="0">
                    <el-collapse v-model="activeNames">
                        <el-collapse-item name="1" style="padding: 0 10px">
                            ...
                            />
                            <el-collapse
                                v-if="Current.componentName != 'ElTable'"
                            >
                                <el-collapse-item
                                    name="1"
                ...
                <el-tab-pane label="样式" name="1">
                    <div style="padding: 9px" class="flex justify-around">
                        <el-button @click="editGlobalStyle" size="small"
                            >编辑全局样式</el-button
                        >
                        <el-button
                            type="primary"
                            @click="editCurrentStyle"
                            size="small"
                            >编辑当前样式</el-button
                        >
                    </div>
                        ...
                        <el-collapse-item name="0" style="padding: 0 10px">
                            ...
                            <div>
                                <div v-if="!currEventMap.length">
                                    点击<strong style="color: green"
                                        >绑定事件</strong
                                    >为画布中所选元素增加事件
                                </div>
                                <div
                                    v-else
                                    v-for="item in currEventMap"
                                    style="
                        ...
                        <el-collapse-item name="2" style="padding: 0 10px">
                            ...
                            <ul v-if="!!PageConfig.popup?.length">
                                ...
                                >
                                    {{ item.props.title }}
                                    <div>
                                        <i-ep-edit @click="editPopup(index)" />
                                        <i-ep-delete
        ...
        <div v-else style="color: #777" class="m-auto w-fit">
            点击组件进行属性设置
        </div>
    </div>
    <el-dialog v-model="styleDialogVisible" :title="dialogTitle" width="800">
        <div
            ...
            style="width: 560px; height: 400px; border: 1px solid #c9c8c8"
        ></div>
        <template #footer>
            <div>
    ...
    >
        ...
        <el-container
            :style="{ height: 'calc(50vh - 116px)' }"
            class="justify-between"
        >
            <ul class="base-component">
                <li
                    v-for="item in renderColumnCompLi"
                    :key="item.dataType"
                    :data-type="item.dataType"
                    class="component-item"
                >
                    <i-ep-document width="15px" />
                    <span>{{ item.text }}</span>
                </li>
            </ul>
            <BasicCanvas
    ...
    >
        ...
        <el-radio-group v-model="marginValue" @change="changeMargin">
            <el-radio-button
                v-for="item in ['auto', 0, 10, 20, 40, 60, 100, 140, 200]"
                :label="item"
    ...
    <!-- 数据请求设置 -->
    <el-dialog
        v-model="fetchDataDialogVisible"
        title="fetch请求设置"
        width="800"
    >
        ...
        <template #footer>
            <div>
                <el-button type="primary" @click="saveFetchData"
                    >保存</el-button
                >
                <el-button @click="fetchDataDialogVisible = false"
                    >取消</el-button
                >
            </div>
        </template>
...
import { renderColumnCompLi, camelize, hyphenate } from '@/mool/utils';
                ...
                if (child.children?.length == 1) {
                    if (
                        formColumns.value.find(
                            (item) => item.title == child.props?.label,
                        )
                    ) {
                        return;
                    }
                    const checked = Current.value.props?.columns?.some(
                        (item) =>
                            item.dataIndex == (child.children ?? [])[0].key,
                    );
...
const fetchData = computed<typeof Current.value.fetchData>(() => {
    return {
        order: null,
        params: {
            method: requesttype.find(
                (item) => item.value == defaultRequestType.value,
            )?.label,
            headers: {
                'content-type': contentType.find(
                    (item) => item.value == defaultContentType.value,
                )?.label,
            },
        },
    };
...
const saveFetchData = () => {
    const result =
        new Function(`return ${pageSchema.value.split('=')[1]}`)() ?? {};
    ...
    if (Current.value.componentName == 'ElTable' && fetchData.value) {
        if (!Current.value.fetchData) {
            if (fetchData.value?.order !== null) {
                ...
                PageConfig.value.ref.searchParams.value[
                    fetchData.value?.order
                ] = params;
            } else {
...
const addPopup = () => {
    PageConfig.value.ref[
        `dialogVisible${PageConfig.value.popup?.length || ''}`
    ] = false;
...
const currEventMap = computed(() => {
    console.log(
        Object.keys(Current.value.props ?? {}).filter((val) =>
            val.startsWith('on'),
        ),
    );

    return Object.keys(Current.value.props ?? {}).filter((val) =>
        val.startsWith('on'),
    );
});
    ...
    (n, o) => {
        if (!n && Current.value.props?.columns) {
            const { title, dataIndex } =
                Current.value.props.columns[currColIndex.value ?? 0];
...
const saveStyle = () => {
    ...
    if (dialogTitle.value === 'Css编辑') {
        ...
        if (matched) {
            ...
            // 创建 <style> 元素并插入样式
            const getStyle = document.querySelector(
                `style[data-id="${props.pageConfig.id}"]`,
            );
            if (styleEle || getStyle) {
                document.querySelector(
                    `style[data-id="${props.pageConfig.id}"]`,
                )!.innerHTML = props.pageConfig.css;
            } else {
    ...
    if (dialogTitle.value === '表头样式编辑') {
        matched = pageSchema.value.match(/:root\s*{([^}]*)}/s);
        Current.value.props &&
            (style = Current.value.props.headerCellStyle = {});
    }
...
const saveRender = () => {
    ...
    if (currPopupIndex.value != null) {
        PageConfig.value.popup[currPopupIndex.value].children = schema.children;
    }
        ...
        over(el) {},
            ...
            switch (Current.value.componentName) {
                case 'ElMenu':
                    ...
                    Current.value.props?.option?.splice(
                        insertIndex.value!,
                        0,
                        option,
                    );
                    break;
                case 'ElTable':
                    const column = (Current.value.props?.columns ?? [])[
                        sourceIndex!
                    ];
                    Current.value.props?.columns?.splice(sourceIndex!, 1);
                    Current.value.props?.columns?.splice(
                        insertIndex.value!,
                        0,
                        column,
                    );
...
const load = () => {
    ...
    fetch(`https://picsum.photos/v2/list?page=${page.value}`, {
        ...
        .then((res) => {
            items.value = [
                ...items.value,
                ...res.map((item) => item.download_url),
            ];
            page.value++;
        })
        .catch((error) => {
                    ...
                    return (
                        <>
                            <el-input
                                ...
                                onUpdate:modelValue={(e: string) => {
                                    if (
                                        PageConfig.value.ref[data.value] !==
                                        undefined
                                    ) {
                                        delete PageConfig.value.ref[data.value];
                                    }
                                    data.value = e;
                                    PageConfig.value.ref[`${e}`] = false;
                                }}
                            ></el-input>
                        </>
                    );
                ...
                render(data, col) {
                    return (
                        <>
                            ...
                            typeof Current.value.props?.class == 'string' ? (
                                <el-input
                                    v-model={(Current.value.props ?? {}).class}
                                ></el-input>
                            ) : (
                                <el-tag
                                    onClose={() => {
                    ...
                    return (
                        <>
                            {!Current.value.props?.ref ? (
                                <el-input
                                    v-model={(Current.value.props ?? {}).ref}
                                ></el-input>
                            ) : (
                                <el-tag
                                    onClose={() => {
                    ...
                    return (
                        <>
                            <div style={{ width: '100%' }} class="radioGroup">
                                {Current.value.props?.option?.map(
                                    (opt, index) => {
                                        return (
                                            <div
                                                style={{
                            ...
                            <el-button
                                ...
                                text
                                onClick={() => {
                                    Current.value.props?.option?.push({
                                        title: '',
                                        index: Current.value.props?.option
                                            ?.length,
                                    });
                                }}
                            >
                ...
                render(data, col) {
                    return (
                        <>
                            ...
                            typeof Current.value.props?.value == 'string' ? (
                                <el-input
                                    v-model={(Current.value.props ?? {}).value}
                                ></el-input>
                            ) : (
                                <el-tag
                                    onClose={() => {
            ...
            },
            {
                        ...
                        <el-select
                            ...
                            v-slots={{
                                header: () => {
                                    return (
                                        <el-checkbox
                                            modelValue={formColumns.value.some(
                                                    ...
                                                    formColumns.value.forEach(
                                                        (item) => {
                                                            ...
                                                            (
                                                                ...
                                                                Current.value.props?.columns?.filter(
                                                                    (i: any) =>
                                                                        i.dataIndex !==
                                                                        item.dataIndex,
                                                                );
                ...
                render(data, col) {
                    return (
                        <>
                            <div style={{ width: '100%' }} class="radioGroup">
                                {Current.value.props?.columns?.map(
                                    (opt, index) => {
                                        return (
                                            <div
                                                style={{
                        ...
                        <>
                                ...
                                <el-tag
                                    onClose={() => {
                                        if (
                                            ...
                                            delete PageConfig.value.methods[
                                                `onSearch${(Current.value.fetchData?.order ?? 0) || ''}`
                                            ];
                                            ...
                                            delete PageConfig.value.methods[
                                                `getOption${(Current.value.fetchData?.order ?? 0) || ''}`
                                            ];
                                            delete Current.value.fetchData;
                                            if (optionIndex === 0) {
                                                optionIndex = null;
                            ...
                            <el-button
                                onClick={async () => {
                                    ...
                                    pageSchema.value = '';
                                    currParamsConf.value =
                                        Current.value.fetchData?.params ||
                                        currParamsConf.value;
                                    defaultContentType.value =
                                        contentType.find(
                                            (item) =>
                                                item.label ==
                                                (currParamsConf.value.headers ??
                                                    {})['content-type'],
                                        )?.value ?? 1;
                    ...
                    return (
                        <el-button
                            onClick={async () => {
                                ...
                                styleDialogVisible.value = true;
                                Object.keys(
                                    Current.value.props?.headerCellStyle,
                                ).map((key) => {
                            ...
                            <el-switch
                                ...
                                onUpdate:modelValue={(e: boolean) => {
                                    ...
                                    if (e) {
                                        ...
                                        PageConfig.value.methods[
                                            `onSearch${Current.value.pagerConfig?.order || ''}`
                                        ] = {
                                            type: 'JSFunction',
