...
function scale$2(data, width, height, newData, newWidth, newHeight) {
  ...
  const filter = function filter(dstCol, dstRow) {
    ...
    for (let i = 0; i <= 3; i += 1) {
      ...
      for (let m = -1; m <= 2; m += 1) {
        for (let n = -1; n <= 2; n += 1) {
          tmpPixels[m + 1][n + 1] = getRGBAValue$1(
            data,
            width,
            height,
            intRow + m,
...
function scaleMixin(ImageScale) {
  ...
  api.scaleImage = function scaleImage(image, args) {
    ...
    const newImageData = ctxTransfer.createImageData(
      finalArgs.width,
      finalArgs.height,
    );
    ...
    ctxTransfer.putImageData(
      newImageData,
      0,
      0,
      0,
      0,
      canvasTransfer.width,
      canvasTransfer.height,
    );
