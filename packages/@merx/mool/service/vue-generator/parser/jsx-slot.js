const { BUILTIN_COMPONENT_NAME, TINY_ICON } = require("../constant");
const { addIconRecord, handleIconInProps } = require("../utils");
...
function handleExprBinding(key, item, attrsArr) {
  ...
  if (item.params?.length) {
    ...
    return attrsArr.push(
      `${key}={(...eventArgs) => ${propValue}(eventArgs, ${extendParams})}`
    );
  }
...
const generateJSXTemplate = (item, description) => {
  ...
  if (condition) {
    const conditionValue = condition?.type
      ? condition.value.replace(/this\./g, "")
      : condition;
  ...
  if (VOID_ELEMENTS.includes(component)) {
    ...
    if (Array.isArray(children)) {
      const subTemplate = children
        .map((child) => generateJSXTemplate(child, description))
        .join("");
...
module.exports = { generateJSXTemplate };
