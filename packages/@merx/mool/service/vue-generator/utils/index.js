...
const { tinyIcon } = require("../pre-processor");
const { TINY_ICON, JS_FUNCTION } = require("../constant");
 ...
 */
const toPascalCase = (input, delimiter = "-") =>
  input.split(delimiter).map(capitalize).join("");
...
const thisBindRe = /this\.(props\.)?/g;
const thisPropsBindRe = /this\.(props\.)?/g;
const thisRegexp = /this\./g;
...
const handleIconInProps = (description, iconProp) => {
  const { componentName: iconName } = tinyIcon(iconProp);
