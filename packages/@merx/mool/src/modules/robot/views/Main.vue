    ...
    <div class="chat-history" ref="chatHistory">
      <div v-for="(message, index) in messages" :key="index" :class="['message-bubble', message.role]">
        <div :class="['message-content', message.role]">
          <span>
            <el-icon v-if="message.role === 'user'" style="width: 25px; height: 25px; vertical-align: text-top">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" style="width: 100%; height: 100%">
                ...
                <path fill="#424242" d="M22 4c-6.1 0-10 4.9-10 11v2.3l2 1.7v-5l12-4l4 4v5l2-1.7V15c0-4-1-8-6-9l-1-2z" />
                ...
                <path fill="#009688" d="M27 31s-1.8 2-5 2s-5-2-5-2S6 33 6 44h32c0-11-11-13-11-13" />
            ...
            <el-icon v-if="message.role === 'system'" style="width: 25px; height: 25px; vertical-align: text-top">
          ...
          <div>
            <p v-if="message.role === 'user'" style="
  ...
  setup(props, { emit }) {
    ...
    const messages = ref<Record<string, any>[]>([
      {
        ...
        content: "你好！有什么我可以帮助你的吗？我可以为你绑定v-model变量等其他变量(需要您提供变量名)",
    ...
    const sendMessage = () => {
      ...
      fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          messages: `请分析当前组件schema：${JSON.stringify(props.schema)}\n` + input,
        ...
        .then((res) => {
          ...
          messages.value[messages.value.length - 1].content = "绑定完成";
